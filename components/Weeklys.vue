...
export default {
    props: {
        items: {
            type: Array as PropType<Array<Weekly>>,  // Notice this type is now for regions
            required: true,
        },
    },
    methods: {
        setActive(item: Weekly) {
            this.items.forEach((i) => i !== item ? i.IsVisible = false : i.IsVisible = !item.IsVisible);
        },
        getImageStyling(item: Weekly) {
            if (item.Image) {
                return {
...
<template>
    <div class="text-center">
        <!-- Weekly Rerun Selector: List of Regions -->
